function DetectRS(filename, path)
...
%         output = strcat ('SingleZ\', filename(1:idx), sprintf('c%d',c), sprintf('p%d',z), '.tiff');
        ...
        switch InverColour
                case 0
                    bfsave(uint16((matxyt)), outputpath);                  % --- no inversion of colour
                case 1
                    bfsave(uint16(imcomplement(matxyt)), outputpath);      % --- do inversion of colour
        end
